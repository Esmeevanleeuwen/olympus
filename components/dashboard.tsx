...
type State = { screen: "workspace" | "audiences" | "layout"; platform: string; provider: string; source: string;
  ...
  layoutTab: "controls" | "suite"; audienceTab: "supabase" | "vercel" | "erd"; storage: boolean; error: string; notice: string; importing: boolean };
...
export default class Dashboard extends React.Component<{ initialSnapshot?: Snapshot }, State> {
  ...
  render() {
    ...
    return <div className="app-shell"><a className="skip-link" href="#workspace-main">Skip to workspace</a>
      ...
      <main id="workspace-main" tabIndex={-1}><div className="page-heading"><div><div className="eyebrow">{s.screen === "audiences" ? (s.audienceTab === "vercel" ? "VERCEL · WEBSITE TRAFFIC" : s.audienceTab === "erd" ? "SUPABASE · DATABASE STRUCTURE" : "SUPABASE · SAVED SNAPSHOT") : "YOUR SPACE, STILL TAKING SHAPE"}</div><h1>{s.screen === "workspace" ? "Your workspace." : s.screen === "audiences" ? "Audiences." : "Make it yours."}</h1><p>{s.screen === "workspace" ? "A simple overview. Room to build on." : s.screen === "audiences" ? (s.audienceTab === "vercel" ? "See how people use your platform." : s.audienceTab === "erd" ? "Explore your tables and their relationships." : "Compare stored table counts by project.") : "A few controls. Nothing complicated."}</p></div>
        ...
        {s.screen === "audiences" && <div className="source-tabs" role="tablist" aria-label="Audience sources" onKeyDown={event => {
          ...
          const tabs = ["supabase", "vercel", "erd"] as const;
        ...
        }}>{(["supabase", "vercel", "erd"] as const).map(tab => <button key={tab} id={`audience-tab-${tab}`} role="tab" aria-controls={`audience-panel-${tab}`} aria-selected={s.audienceTab === tab} tabIndex={s.audienceTab === tab ? 0 : -1} className={s.audienceTab === tab ? "selected" : ""} onClick={() => this.setState({ audienceTab: tab, error: "", notice: "" })}>{tab === "supabase" ? "Supabase" : tab === "vercel" ? "Vercel Analytics" : "ERD"}</button>)}</div>}
