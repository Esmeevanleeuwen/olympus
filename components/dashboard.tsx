...
export default class Dashboard extends React.Component<{ initialSnapshot?: Snapshot }, State> {
  ...
  render() {
    ...
    return <div className="app-shell"><a className="skip-link" href="#workspace-main">Skip to workspace</a>
      ...
      <main id="workspace-main" tabIndex={-1}><div className="page-heading"><div><div className="eyebrow">{s.screen === "audiences" ? "SUPABASE · SAVED SNAPSHOT" : "YOUR SPACE, STILL TAKING SHAPE"}</div><h1>{s.screen === "workspace" ? "Your workspace." : s.screen === "audiences" ? "Audiences." : "Make it yours."}</h1><p>{s.screen === "workspace" ? "A simple overview. Room to build on." : s.screen === "audiences" ? "Compare stored table counts by project." : "A few controls. Nothing complicated."}</p></div>
        <div className="page-actions">{s.screen !== "layout" && <button className="button" onClick={() => this.fileInput?.click()} disabled={s.importing}><Icon name="upload" size={16}/>{s.importing ? "Reading…" : "Import snapshot"}</button>}{s.screen === "workspace" && <button className="button primary" onClick={() => this.addBlock(group)}><Icon name="plus" size={16}/>Add block</button>}</div></div>
        ...
        {s.screen === "audiences" && <div className="audiences-dashboard">
