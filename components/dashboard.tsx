    ...
    return <div className="app-shell"><a className="skip-link" href="#workspace-main">Skip to workspace</a>
      ...
      <main id="workspace-main" tabIndex={-1}><div className="page-heading"><div><div className="eyebrow">{s.screen === "audiences" ? (s.audienceTab === "vercel" ? "VERCEL · WEBSITE TRAFFIC" : "SUPABASE · SAVED SNAPSHOT") : "YOUR SPACE, STILL TAKING SHAPE"}</div><h1>{s.screen === "workspace" ? "Your workspace." : s.screen === "audiences" ? "Audiences." : "Make it yours."}</h1><p>{s.screen === "workspace" ? "A simple overview. Room to build on." : s.screen === "audiences" ? (s.audienceTab === "vercel" ? "See how people use your platform." : "Compare stored table counts by project.") : "A few controls. Nothing complicated."}</p></div>
        ...
        {s.screen === "audiences" && <div className="source-tabs" role="tablist" aria-label="Audience sources" onKeyDown={event => {
          ...
          const audienceTab = event.key === "Home" ? "supabase" : event.key === "End" ? "vercel" : s.audienceTab === "supabase" ? "vercel" : "supabase";
          this.setState({ audienceTab }, () => document.getElementById(`audience-tab-${audienceTab}`)?.focus());
        }}>{(["supabase", "vercel"] as const).map(tab => <button key={tab} id={`audience-tab-${tab}`} role="tab" aria-controls={`audience-panel-${tab}`} aria-selected={s.audienceTab === tab} tabIndex={s.audienceTab === tab ? 0 : -1} className={s.audienceTab === tab ? "selected" : ""} onClick={() => this.setState({ audienceTab: tab, error: "", notice: "" })}>{tab === "supabase" ? "Supabase" : "Vercel Analytics"}</button>)}</div>}
