...
import { Block, Layout, Resource, Snapshot, Provider, SuiteTool, SidebarConfig, PLATFORMS, PROVIDERS, INITIAL_LAYOUT,
  parseSnapshot, parseLayout, resourcesWithSnapshot, filterResources, chartMaximum, moveBlock, buildComponents, componentSimilarity, type WorkspaceComponent } from "../lib/workspace";
...
type State = { screen: "features" | "components" | "workspace" | "audiences" | "layout"; platform: string; provider: string; source: string;
  search: string; chart: boolean; expanded: boolean; snapshot: Snapshot | null; layout: Layout; modal: ModalState;
  layoutTab: "controls" | "suite" | "sidebar"; dataWorkspace: DataWorkspaceState | null; tableRequest: TableRequest | null; audienceTab: "data" | "supabase" | "vercel" | "erd"; storage: boolean; error: string; notice: string; importing: boolean; selectedComponentId: string | null };
...
function ResourceList({ rows, chart, maximum, onSelect }: { rows: Resource[]; chart: boolean; maximum: number; onSelect: (resource: Resource) => void }) {
  return <div className="resource-table-wrap"><table className="resource-table"><thead><tr><th>Name</th><th className="source-column">Source</th><th className="value-heading">{chart ? `Rows Â· scale 0â€“${number.format(maximum)}` : "Rows"}</th><th><span className="sr-only">Details</span></th></tr></thead><tbody>{rows.map(row => <tr key={row.id}>
    ...
    <td className="value-cell">{chart && <span className="bar-track" aria-hidden="true"><span style={{ width: `${100 * (row.count ?? 0) / maximum}%` }}/></span>}<span className="count">{row.count === null ? "â€”" : number.format(row.count)}</span></td>
...
function confidenceLabel(confidence: WorkspaceComponent["confidence"]) {
  return confidence === "strong" ? "Strong match" : confidence === "confirmed" ? "Confirmed" : "Possible match";
}
function ComponentEvidenceRow({ evidence, onOpen }: { evidence: WorkspaceComponent["evidence"][number]; onOpen: () => void }) {
  const resource = evidence.resource;
  return <button className="component-evidence" onClick={onOpen}>
    <ProviderMark provider={resource.provider}/><span className="component-evidence-copy"><strong>{resource.name}</strong><small>{PROVIDERS[resource.provider]} Â· {resource.source}</small></span>
    <span className="component-evidence-value">{resource.count === null ? resource.kind : `${number.format(resource.count)} rows`}</span><Icon name="arrow" size={15}/>
  </button>;
}
function ComponentView({ components, search, selectedId, onSearch, onSelect, onOpenResource }: {
  components: WorkspaceComponent[]; search: string; selectedId: string | null; onSearch: (value: string) => void;
  onSelect: (id: string) => void; onOpenResource: (resource: Resource) => void;
}) {
  const filtered = components.filter(component => `${component.name} ${component.evidence.map(item => `${item.resource.name} ${item.resource.source} ${item.resource.provider}`).join(" ")}`.toLowerCase().includes(search.trim().toLowerCase()));
  const selected = components.find(component => component.id === selectedId) ?? filtered[0] ?? components[0];
  const similar = selected ? components.filter(component => component.id !== selected.id).map(component => ({ component, score: componentSimilarity(selected, component) })).filter(item => item.score >= 0.5).sort((a, b) => b.score - a.score).slice(0, 3) : [];
  return <div className="components-view">
    <div className="component-summary"><div><span className="eyebrow">CROSS-PLATFORM MAP</span><h2>See what belongs together.</h2><p>Olympus groups matching names across your database, code and traffic. Every connection keeps its source.</p></div><div className="component-count"><strong>{components.length}</strong><span>components<br/>from {new Set(components.flatMap(component => component.evidence.map(item => item.resource.provider))).size} sources</span></div></div>
    <div className="component-toolbar"><label className="search-input"><Icon name="search" size={16}/><span className="sr-only">Search components</span><input placeholder="Find a componentâ€¦" value={search} onChange={event => onSearch(event.target.value)}/>{search && <button className="icon-button" aria-label="Clear component search" onClick={() => onSearch("")}><Icon name="close" size={14}/></button>}</label><span className="component-legend"><span className="legend-dot strong"/>Strong match <span className="legend-dot possible"/>Possible match</span></div>
    {selected ? <div className="component-layout"><section className="component-list" aria-label="Components">{filtered.length ? filtered.map(component => <button key={component.id} className={`component-row ${component.id === selected.id ? "selected" : ""}`} onClick={() => onSelect(component.id)}><span className="component-row-main"><strong>{component.name}</strong><small>{component.evidence.length} evidence {component.evidence.length === 1 ? "source" : "sources"}</small></span><span className={`confidence confidence-${component.confidence}`}>{confidenceLabel(component.confidence)}</span></button>) : <EmptyState title="No matching components">Try another name or clear the search.</EmptyState>}</section>
      <aside className="component-inspector" aria-labelledby="component-inspector-title"><div className="component-inspector-heading"><div><span className="eyebrow">INSPECTOR</span><h2 id="component-inspector-title">{selected.name}</h2></div><span className={`confidence confidence-${selected.confidence}`}>{confidenceLabel(selected.confidence)}</span></div><p className="component-explanation">{selected.evidence.length > 1 ? "These sources share a component name. Confirm the relationship before treating it as a live connection." : "This source has no matching evidence yet. Similar names can suggest a lead, not a confirmed connection."}</p><div className="component-evidence-list">{selected.evidence.map(evidence => <ComponentEvidenceRow key={evidence.resource.id} evidence={evidence} onOpen={() => onOpenResource(evidence.resource)}/>)}</div>{similar.length > 0 && <div className="similar-section"><div className="panel-heading"><div><h3>Similar components</h3><p>Shared naming signals, not a confirmed link.</p></div></div>{similar.map(item => <button className="similar-row" key={item.component.id} onClick={() => onSelect(item.component.id)}><span><strong>{item.component.name}</strong><small>{Math.round(item.score * 100)}% shared naming signals</small></span><Icon name="arrow" size={15}/></button>)}</div>}</aside>
    </div> : <section className="panel component-empty"><EmptyState title="No evidence loaded" action={<span>Import a snapshot from Overview to compare sources.</span>}>Olympus needs resource references before it can form a component view.</EmptyState></section>}
  </div>;
}

type DashboardProps = { initialSnapshot?: Snapshot; dataWorkspaceUrl?: string };

class DashboardContent extends React.Component<DashboardProps & { access: ReturnType<typeof useWorkspaceAccess> }, State> {
  fileInput: HTMLInputElement | null = null;
  tableRequestId = 0;
  constructor(props: DashboardProps & { access: ReturnType<typeof useWorkspaceAccess> }) {
    super(props); this.state = { screen: "workspace", platform: "all", provider: props.initialSnapshot ? "supabase" : "all", source: props.initialSnapshot?.resources.find(r => r.provider === "supabase")?.source || "all", search: "", chart: false,
      ...
      layout: INITIAL_LAYOUT, dataWorkspace: null, tableRequest: null, modal: null, layoutTab: "controls", audienceTab: "data", storage: false, error: "", notice: "", importing: false, selectedComponentId: null };
  }
  componentDidMount() {
  ...
  collapseSidebar = (collapsed: boolean) => this.props.access.setCollapsed(collapsed);
  ...
  renderModal() {
    ...
    if (modal.type === "resource") { const r = modal.resource; return <Modal title={r.name} onClose={close}>
      <span className="tag"><ProviderMark provider={r.provider}/>{PROVIDERS[r.provider]} Â· {r.kind}</span>
      <dl className="metadata"><dt>Source</dt><dd>{r.source}</dd><dt>Platform</dt><dd>{PLATFORMS.find(p => p.id === r.platform)?.name || "Not assigned"}</dd><dt>Stored rows</dt><dd>{r.count === null ? "Not available" : number.format(r.count)}</dd><dt>Retrieved</dt><dd>{r.capturedAt ? `${date(r.capturedAt)} Â· ${new Date(r.capturedAt).toISOString().slice(11, 16)} UTC` : "Reference only"}</dd></dl>
    ...
    return <Modal title={modal.group === "audiences" ? "Audience draft" : "Edit block"} onClose={close}><form onSubmit={event => { event.preventDefault(); this.saveBlock(); }}>
      ...
      <label className="field">Content type<select value={modal.draft.kind} onChange={event => this.updateDraft({ kind: event.target.value as Block["kind"] })}><option value="blank">Blank â€” decide later</option><option value="note">Note</option></select></label>
      {modal.draft.kind === "note" && <label className="field">Draft text<textarea rows={5} maxLength={4000} placeholder="Lorem ipsum dolor sit ametâ€¦" value={modal.draft.text} onChange={event => this.updateDraft({ text: event.target.value })}/></label>}
  ...
  render() {
    const s = { ...this.state, layout: { ...this.state.layout, sidebar: { ...this.state.layout.sidebar, collapsed: this.props.access.collapsed, width: this.props.access.preferences.width } } }, all = resourcesWithSnapshot(s.snapshot);
    ...
    return <div className={`app-shell has-simple-sidebar ${s.layout.sidebar.collapsed ? "sidebar-is-collapsed" : ""}`} style={{ "--os-width": s.layout.sidebar.width === "wide" ? "280px" : "240px", "--sidebar-size": s.layout.sidebar.width === "wide" ? "280px" : "240px", "--sidebar-space": s.layout.sidebar.collapsed ? "0px" : s.layout.sidebar.width === "wide" ? "280px" : "240px" } as React.CSSProperties}><a className="skip-link" href="#workspace-main">Skip to workspace</a>
      ...
      <div className="app-body"><header className="topbar"><div className="platform-control"><Icon name="grid" size={16}/><label className="sr-only" htmlFor="platform">Filter resource platform</label><select id="platform" disabled={s.screen !== "workspace"} title="Filters resource references only" value={s.platform} onChange={event => this.setState({ platform: event.target.value, source: "all", search: "", expanded: false })}><option value="all">All platforms</option>{PLATFORMS.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
        <div className="topbar-tools"><label className="topbar-search"><Icon name="search" size={15}/><span className="sr-only">Search this view</span><input value={s.search} placeholder={s.screen === "components" ? "Search components…" : "Search resources…"} onChange={event => this.setState({ search: event.target.value, expanded: false })}/>{s.search && <button className="icon-button" aria-label="Clear search" onClick={() => this.setState({ search: "" })}><Icon name="close" size={13}/></button>}</label><details className="topbar-menu"><summary className="topbar-menu-trigger"><Icon name="sliders" size={14}/>View</summary><div className="topbar-menu-panel">
          {s.screen === "workspace" && <><span className="topbar-menu-label">Source</span>{["all", "supabase", "github", "vercel"].map(provider => <button key={provider} className={s.provider === provider ? "is-selected" : ""} onClick={() => this.selectProvider(provider)}>{provider === "all" ? "All sources" : PROVIDERS[provider as Provider]}{s.provider === provider && <Icon name="check" size={13}/>}</button>)}</>}
          {s.screen === "audiences" && <><span className="topbar-menu-label">Audience view</span>{(["data", "supabase", "vercel", "erd"] as const).map(tab => <button key={tab} className={s.audienceTab === tab ? "is-selected" : ""} onClick={() => this.setState({ audienceTab: tab })}>{tab === "data" ? "Demo data" : tab === "supabase" ? "Supabase snapshot" : tab === "vercel" ? "Vercel Analytics" : "Supabase ERD"}{s.audienceTab === tab && <Icon name="check" size={13}/>}</button>)}</>}
          {s.screen === "layout" && <><span className="topbar-menu-label">Layout section</span>{(["controls", "sidebar", "suite"] as const).map(tab => <button key={tab} className={s.layoutTab === tab ? "is-selected" : ""} onClick={() => this.setState({ layoutTab: tab })}>{tab === "controls" ? "Layout & data" : tab === "sidebar" ? "Sidebar" : "Suite sidebar"}{s.layoutTab === tab && <Icon name="check" size={13}/>}</button>)}</>}
          {s.screen !== "workspace" && s.screen !== "audiences" && s.screen !== "layout" && <span className="topbar-menu-empty">This view has no extra filters.</span>}
        </div></details></div>
        <div className="topbar-right"><span className="preview-tag">{s.screen === "features" ? "Live instellingen" : "Design preview"}</span><span className="top-divider"/><span className="owner-avatar">Z</span><div className="owner">Zeus<small>Owner</small></div></div></header>
      <main id="workspace-main" tabIndex={-1} className={s.screen === "audiences" && s.audienceTab === "data" ? "data-workspace-main" : undefined}><div className="page-heading"><div><div className="eyebrow">{s.screen === "audiences" ? (s.audienceTab === "data" ? "DATA Â· DEMO WORKSPACE" : s.audienceTab === "vercel" ? "VERCEL Â· WEBSITE TRAFFIC" : s.audienceTab === "erd" ? "SUPABASE Â· DATABASE STRUCTURE" : "SUPABASE Â· SAVED SNAPSHOT") : "YOUR SPACE, STILL TAKING SHAPE"}</div><h1>{s.screen === "features" ? "Functies per platform." : s.screen === "components" ? "Your components." : s.screen === "workspace" ? "Your workspace." : s.screen === "audiences" ? "Audiences." : "Make it yours."}</h1><p>{s.screen === "features" ? "Eén gedeelde basis. Jij bepaalt waar die beschikbaar is." : s.screen === "components" ? "One view across your data, code and live signals." : s.screen === "workspace" ? "A simple overview. Room to build on." : s.screen === "audiences" ? (s.audienceTab === "data" ? "Explore tables, connections and query drafts." : s.audienceTab === "vercel" ? "See how people use your platform." : s.audienceTab === "erd" ? "Explore your tables and their relationships." : "Compare stored table counts by project.") : "A few controls. Nothing complicated."}</p></div>
        <div className="page-actions">{(s.screen === "workspace" || (s.screen === "audiences" && s.audienceTab === "supabase")) && <button className="button" onClick={() => this.fileInput?.click()} disabled={s.importing}><Icon name="upload" size={16}/>{s.importing ? "Readingâ€¦" : "Import snapshot"}</button>}{s.screen === "workspace" && <button className="button primary" onClick={() => this.addBlock(group)}><Icon name="plus" size={16}/>Add block</button>}</div></div>
        ...
        {s.screen === "features" && <PlatformFeatures/>}
        {s.screen === "components" && <ComponentView components={buildComponents(all)} search={s.search} selectedId={s.selectedComponentId} onSearch={search => this.setState({ search })} onSelect={selectedComponentId => this.setState({ selectedComponentId })} onOpenResource={resource => this.setState({ modal: { type: "resource", resource } })}/>}
        ...
        {s.screen === "audiences" && s.audienceTab === "supabase" && <div className="audiences-dashboard" id="audience-panel-supabase" role="tabpanel" aria-labelledby="audience-tab-supabase">
          <section className="audience-project-row" aria-label="Audience project"><label><span>Supabase project</span><select value={audienceSource} disabled={sources.length === 0} onChange={event => this.setState({ source: event.target.value, search: "", expanded: false })}>{sources.length === 0 ? <option value="all">No project loaded</option> : sources.map(source => <option key={source} value={source}>{source}</option>)}</select></label><span className="snapshot-state"><span className="neutral-dot"/>Snapshot Â· not live</span></section>
          {audienceScoped.length > 0 ? <>
            <div className="audience-totals" aria-label="Table counts">{audienceHighlights.map(resource => <article key={resource.id}><button className="audience-summary" onClick={() => this.setState({ modal: { type: "resource", resource } })}><span>{resource.name}</span><strong>{resource.count === null ? "â€”" : number.format(resource.count)}</strong><small>{resource.count === null ? "Count unavailable" : "Stored rows"}</small></button></article>)}</div>
            <section className="panel audience-data-panel" aria-labelledby="audience-tables-title"><div className="panel-heading"><div><h2 id="audience-tables-title">Supabase tables <span className="small-count">{audienceScoped.length}</span></h2><p>Exact stored rows from the selected project.</p></div></div><div className="table-toolbar"><label className="search-input"><Icon name="search" size={16}/><span className="sr-only">Search audience tables</span><input placeholder="Find a tableâ€¦" value={s.search} onChange={event => this.setState({ search: event.target.value, expanded: false })}/>{s.search && <button className="icon-button" aria-label="Clear search" onClick={() => this.setState({ search: "" })}><Icon name="close" size={14}/></button>}</label></div>
